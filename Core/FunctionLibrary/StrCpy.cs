...
namespace CSim.Core.FunctionLibrary {
	...
	public sealed class StrCpy: EmbeddedFunction {
		...
		public override void Execute(RValue[] realParams)
		{
            ...
            if ( paramSrc is TempVariable ) {
                if ( paramSrc.LiteralValue is StrLiteral strLit ) {
                    foreach ( char ch in strLit.Value ) {
                        this.Machine.Memory.Write( dstIndex, new []{ (byte) ch } );

                        ++dstIndex;

                        if ( dstIndex >= this.Machine.Memory.Max ) {
                            throw new Exceptions.IncorrectAddressException(
                    ...
                    // End mark ('\0')
                    this.Machine.Memory.Write( dstIndex, new byte[]{ 0 } );
                } else {
			...
			var result = new NoPlaceTempVariable( paramDst.LiteralValue );
